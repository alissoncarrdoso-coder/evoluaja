...
export default function Treinos() {
  ...
  const excluirEx = (fichaId, exId) => setFichas(p => p.map(f => f.id !== fichaId ? f : { ...f, exercicios: f.exercicios.filter(e => e.id !== exId) }))
  ...
  return (
    <div className="page-container animate-fade-in-up">
      ...
      <div className="card mb-5 flex gap-4">
        <div className="flex-1 text-center"><p className="text-xs font-bold text-gray-400 mb-1">Fichas ativas</p><p className="text-lg font-black text-gray-800 dark:text-gray-100">{fichas.length}</p></div>
        ...
        {fichas.map(ficha => (
          <div key={ficha.id} className="card overflow-hidden">
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => excluirFicha(ficha.id)} className="p-1.5 rounded-xl hover:bg-red-50 dark:hover:bg-red-950"><Trash2 size={15} className="text-red-400" /></button>
            </div>
            {aberto === ficha.id && (
              <div className="mt-4 pt-4 border-t border-gray-50 dark:border-gray-800">
                {ficha.exercicios.map(ex => <ExercicioRow key={ex.id} ex={ex} onEdit={e => abrirEx(ficha.id, e)} onDelete={id => excluirEx(ficha.id, id)} />)}
                <button onClick={() => abrirEx(ficha.id)} className="w-full flex items-center justify-center gap-2 py-3 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-2xl text-gray-400 font-bold text-sm hover:border-brand-300 hover:text-brand-500 transition-colors mt-3">
        ...
        <Modal titulo={modalFicha === 'novo' ? 'Nova ficha' : 'Editar ficha'} onClose={() => setModalFicha(null)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => setModalFicha(null)} className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 text-sm font-bold text-gray-500">Cancelar</button>
      ...
      {modalEx && (
        <Modal titulo={modalEx.exId === 'novo' ? 'Novo exercício' : 'Editar exercício'} onClose={() => setModalEx(null)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => setModalEx(null)} className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 text-sm font-bold text-gray-500">Cancelar</button>
              <button onClick={salvarEx} className="flex-1 btn-primary py-3 flex items-center justify-center gap-2"><Check size={16} /> Salvar</button>
            </div>
          </div>
        </Modal>
      )}
