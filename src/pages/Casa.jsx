...
export default function Casa() {
  ...
  return (
    <div className="page-container animate-fade-in-up">
      ...
      {tab === 1 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => abrirC()} className="btn-primary py-2 px-4 text-xs flex items-center gap-1.5"><Plus size={14} /> Adicionar item</button>
          </div>
          ...
          {compras.length === 0 && (
            <div className="card text-center py-8">
              <ShoppingCart size={32} className="text-gray-200 dark:text-gray-700 mx-auto mb-2" />
              <p className="text-sm font-bold text-gray-400">Lista de compras vazia</p>
            </div>
          )}
        </div>
      )}
      ...
      {modalT && (
        <Modal titulo={modalT.id === 'novo' ? 'Nova tarefa' : 'Editar tarefa'} onClose={() => setModalT(null)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => setModalT(null)} className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 text-sm font-bold text-gray-500">Cancelar</button>
              <button onClick={salvarT} className="flex-1 btn-primary py-3 flex items-center justify-center gap-2"><Check size={16} /> Salvar</button>
            </div>
          </div>
        </Modal>
      )}
      ...
      {modalC && (
        <Modal titulo={modalC === 'novo' ? 'Novo item' : 'Editar item'} onClose={() => setModalC(null)}>
          <div className="space-y-4">
            ...
            <div><label className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1 block">Quantidade</label><input type="text" value={fC.qtd} onChange={e => setFC(p => ({ ...p, qtd: e.target.value }))} placeholder="Ex: 1kg, 2 un, 1 cx" className="input" /></div>
            <div>
