    ...
    <div className="page-container animate-fade-in-up">
      ...
      <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-2xl p-1 mb-5">
        {TABS.map((t, i) => (
          <button key={t} onClick={() => setTab(i)}
            className={`flex-1 py-2 text-xs font-bold rounded-xl transition-all ${tab === i ? 'bg-white dark:bg-gray-700 text-brand-600 shadow-sm' : 'text-gray-500 dark:text-gray-400'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {tab === 0 && (
        <div>
          ...
          {consultas.length === 0 && (
            <div className="card text-center py-8"><Heart size={32} className="text-gray-200 dark:text-gray-700 mx-auto mb-2" /><p className="text-sm font-bold text-gray-400">Nenhuma consulta cadastrada</p></div>
          )}
        </div>
      )}
      ...
      {tab === 1 && (
        <div>
          ...
          <div className="space-y-3">
            ...
            {medicamentos.length === 0 && (
              <div className="card text-center py-8"><Pill size={32} className="text-gray-200 dark:text-gray-700 mx-auto mb-2" /><p className="text-sm font-bold text-gray-400">Nenhum medicamento cadastrado</p></div>
            )}
          </div>
        </div>
      )}
      ...
      {modalC && (
        <Modal titulo={modalC === 'novo' ? 'Nova consulta' : 'Editar consulta'} onClose={() => setModalC(null)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => setModalC(null)} className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 text-sm font-bold text-gray-500">Cancelar</button>
              <button onClick={salvarC} className="flex-1 btn-primary py-3 flex items-center justify-center gap-2"><Check size={16} /> Salvar</button>
            </div>
          </div>
        </Modal>
      )}
      ...
      {modalM && (
        <Modal titulo={modalM === 'novo' ? 'Novo medicamento' : 'Editar medicamento'} onClose={() => setModalM(null)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => setModalM(null)} className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 text-sm font-bold text-gray-500">Cancelar</button>
              <button onClick={salvarM} className="flex-1 btn-primary py-3 flex items-center justify-center gap-2"><Check size={16} /> Salvar</button>
            </div>
          </div>
        </Modal>
      )}
