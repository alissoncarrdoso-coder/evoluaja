...
import { Play, Pause, RotateCcw, BookOpen, Plus, Trash2, Edit2 } from 'lucide-react'
...
const CORES_M = [
  {cor:'text-blue-600',bg:'bg-blue-50 dark:bg-blue-950'},
  {cor:'text-green-600',bg:'bg-green-50 dark:bg-green-950'},
  {cor:'text-amber-600',bg:'bg-amber-50 dark:bg-amber-950'},
  {cor:'text-purple-600',bg:'bg-purple-50 dark:bg-purple-950'},
  {cor:'text-red-600',bg:'bg-red-50 dark:bg-red-950'},
  {cor:'text-pink-600',bg:'bg-pink-50 dark:bg-pink-950'},
]
...
export default function Estudos() {
  ...
  const abrirModal = (m=null) => {
    setForm(m?{nome:m.nome,prioridade:m.prioridade,progresso:m.progresso,cor:m.cor,bg:m.bg}:{nome:'',prioridade:'Média',progresso:0,cor:'text-blue-600',bg:'bg-blue-50 dark:bg-blue-950'})
    setModal(m?m.id:'novo')
  }
  const salvar = () => {
    if (!form.nome) return
    ...
    <div className="page-container animate-fade-in-up">
      ...
      <div className="card mb-5 flex flex-col items-center py-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={reset} className="p-3 rounded-2xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700">
            <RotateCcw size={18} className="text-gray-500 dark:text-gray-400"/>
          </button>
          <button onClick={()=>setRodando(r=>!r)}
            className="px-8 py-3 bg-brand-500 text-white rounded-2xl font-bold flex items-center gap-2 hover:bg-brand-600 shadow-md shadow-brand-200">
      ...
      {modal && (
        <Modal titulo={modal==='novo'?'Nova matéria':'Editar matéria'} onClose={()=>setModal(null)} onConfirm={salvar} confirmLabel="Salvar matéria">
