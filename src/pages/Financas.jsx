      ...
      <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-2xl p-1 mb-5">
        {TABS.map((t, i) => (
          <button key={t} onClick={() => setTab(i)}
            className={`flex-1 py-2 text-[11px] font-bold rounded-xl transition-all ${tab === i ? 'bg-white dark:bg-gray-700 text-brand-600 shadow-sm' : 'text-gray-500 dark:text-gray-400'}`}>
            {t}
          </button>
        ))}
      </div>
            ...
            <div key={c.id} className={`bg-gradient-to-br ${c.cor} rounded-3xl p-5 text-white shadow-lg`}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="font-black text-lg">{c.nome}</p><p className="text-xs text-white/60">{c.bandeira} · Vence dia {c.vencimento}</p></div>
                </div>
