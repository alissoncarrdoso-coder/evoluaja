...
const catColors = {
  ...
  'Lazer':       'bg-cyan-100 text-cyan-700',
...
const tarefasIniciais = [
  ...
  { id: 9, hora: '21:00', titulo: 'Leitura 30 min', cat: 'Hábitos', status: 'pending' },
  { id: 10, hora: '22:30', titulo: 'Dormir', cat: 'Saúde', status: 'pending' },
]

const tarefaVazia = { hora: '08:00', titulo: '', cat: 'Trabalho', status: 'pending' }
...
export default function Rotina() {
  ...
  const toggle = (id) => setTarefas(prev => prev.map(t =>
    t.id === id ? { ...t, status: t.status === 'done' ? 'pending' : 'done' } : t
  ))
  const excluir = (id) => setTarefas(prev => prev.filter(t => t.id !== id))

  const abrirNova = () => { setForm(tarefaVazia); setModal('nova') }
  const abrirEditar = (t) => { setForm({ hora: t.hora, titulo: t.titulo, cat: t.cat, status: t.status }); setModal(t.id) }

  const salvar = () => {
    if (!form.titulo.trim()) return
    if (modal === 'nova') {
      setTarefas(prev => [...prev, { ...form, id: Date.now() }].sort((a, b) => a.hora.localeCompare(b.hora)))
    } else {
      setTarefas(prev => prev.map(t => t.id === modal ? { ...t, ...form } : t))
    }
    setModal(null)
  }
    ...
    <div className="page-container animate-fade-in-up">
      ...
      <div className="card mb-5">
        <div className="flex items-center justify-between mb-3">
          <button className="p-1 rounded-xl hover:bg-gray-100"><ChevronLeft size={18} className="text-gray-400" /></button>
          <p className="font-bold text-gray-700 text-sm">{hoje.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}</p>
          <button className="p-1 rounded-xl hover:bg-gray-100"><ChevronRight size={18} className="text-gray-400" /></button>
        </div>
          ...
          {diasSemana.map((d, i) => {
            ...
            return (
              <button key={d} onClick={() => setDiaAtivo(i)}
                className={clsx('flex flex-col items-center py-2 rounded-2xl transition-all', isActive && !isToday ? 'bg-gray-100' : '', isToday ? 'bg-brand-500' : '')}>
                <span className={clsx('text-[10px] font-bold mb-1', isToday ? 'text-brand-200' : 'text-gray-400')}>{d}</span>
                <span className={clsx('text-sm font-black', isToday ? 'text-white' : 'text-gray-700')}>{hoje.getDate() - diaHojeIdx + i}</span>
              </button>
            )
          })}
        </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-sm font-bold text-gray-500">{concluidas}/{tarefas.length}</span>
      </div>
        ...
        {tarefas.map(t => (
          <div key={t.id} className={clsx('card flex items-center gap-3 py-3 transition-all', t.status === 'done' ? 'opacity-60' : '')}>
            ...
            <div className="flex-1 min-w-0">
              <p className={clsx('text-sm font-bold truncate', t.status === 'done' ? 'line-through text-gray-400' : 'text-gray-800')}>{t.titulo}</p>
            ...
            <button onClick={() => abrirEditar(t)} className="p-1.5 rounded-xl hover:bg-gray-100 flex-shrink-0"><Edit2 size={15} className="text-gray-400" /></button>
            <button onClick={() => excluir(t.id)} className="p-1.5 rounded-xl hover:bg-red-50 flex-shrink-0"><Trash2 size={15} className="text-red-400" /></button>
          </div>
        ))}
      </div>
              ...
              <div className="flex flex-wrap gap-2">
                {CATEGORIAS.map(c => (
                  <button key={c} onClick={() => setForm(p => ({ ...p, cat: c }))}
                    className={clsx('px-3 py-1.5 rounded-xl text-xs font-bold border transition-all', form.cat === c ? 'bg-brand-500 text-white border-brand-500' : 'bg-white text-gray-600 border-gray-200')}>
                    {c}
                  </button>
                ))}
              </div>
