...
import {
  ...
  AlertCircle, Plus, Trash2, Edit2, X, Check, CalendarDays
} from 'lucide-react'
...
// Retorna a data de domingo da semana que contém `date`
...
// Formata Date para chave YYYY-MM-DD
function toKey(date) {
  return date.toISOString().split('T')[0]
}

// Parse YYYY-MM-DD sem timezone shift
...
// Tarefas padrão do dia atual
const hoje = new Date()
const hojeKey = toKey(hoje)

const tarefasDefaultDia = [
...
export default function Rotina() {
  // Todas as tarefas: { [dateKey]: TarefaItem[] }
  const [todasTarefas, setTodasTarefas] = useLocalStorage('evoluaja_tarefas_v2', {
    [hojeKey]: tarefasDefaultDia
  })

  // Semana exibida: domingo da semana
  ...
  const voltarHoje = () => {
    setSemanaBase(inicioSemana(hoje))
    setDiaSelecionado(hojeKey)
  }

  // Tarefas do dia selecionado
  const tarefasDia = todasTarefas[diaSelecionado] || []

  // Helpers para modificar tarefas de um dia específico
  const setTarefasDia = (key, fn) => {
  ...
  const toggle = (id) => {
    setTarefasDia(diaSelecionado, prev =>
      prev.map(t => t.id === id ? { ...t, status: t.status === 'done' ? 'pending' : 'done' } : t)
    )
  }

  const excluir = (id) => {
    setTarefasDia(diaSelecionado, prev => prev.filter(t => t.id !== id))
  }

  const abrirNova = () => {
    setForm({ hora: '08:00', titulo: '', cat: 'Trabalho', status: 'pending' })
    setModal('nova')
  }

  const abrirEditar = (t) => {
    setForm({ hora: t.hora, titulo: t.titulo, cat: t.cat, status: t.status })
    setModal(t.id)
  }
  ...
  const salvar = () => {
    ...
    if (modal === 'nova') {
      setTarefasDia(diaSelecionado, prev =>
        [...prev, { ...form, id: Date.now() }].sort((a, b) => a.hora.localeCompare(b.hora))
      )
    } else {
      setTarefasDia(diaSelecionado, prev =>
        prev.map(t => t.id === modal ? { ...t, ...form } : t)
      )
    }
    setModal(null)
  }
  ...
  // Mês/ano exibido no header do calendário
  // Mostra o mês do dia selecionado, ou o mês da semana
  const diaSel = fromKey(diaSelecionado)
  const mesAno = `${meses[diaSel.getMonth()]} ${diaSel.getFullYear()}`

  // Verifica se um dia tem tarefas
  const temTarefas = (key) => (todasTarefas[key] || []).length > 0
  const qtdTarefas = (key) => (todasTarefas[key] || []).length
  const qtdConcluidas = (key) => (todasTarefas[key] || []).filter(t => t.status === 'done').length

  const chip = (ativo) => clsx(
  ...
  return (
    <div className="page-container animate-fade-in-up">
      ...
      <div className="card mb-5">
        ...
        <div className="grid grid-cols-7 gap-1">
          {diasDaSemana.map((dia, i) => {
            ...
            const total = qtdTarefas(key)
            const done = qtdConcluidas(key)
            ...
            return (
              <button
                ...
                className={clsx(
                  'flex flex-col items-center py-2 rounded-2xl transition-all relative',
                  isSelected && isToday  ? 'bg-brand-500' : '',
                  isSelected && !isToday ? 'bg-brand-100 dark:bg-brand-900' : '',
                  !isSelected && isToday  ? 'ring-2 ring-brand-400' : '',
                  !isSelected && !isToday ? 'hover:bg-gray-50 dark:hover:bg-gray-800' : '',
                )}
              >
                {/* Rótulo do dia */}
                <span className={clsx(
                  'text-[10px] font-bold mb-1',
                  isSelected && isToday   ? 'text-brand-100' : '',
                  isSelected && !isToday  ? 'text-brand-600 dark:text-brand-300' : '',
                  !isSelected && isToday  ? 'text-brand-500' : '',
                  !isSelected && !isToday ? 'text-gray-400 dark:text-gray-500' : '',
                )}>
                  {diasSemana[i]}
                </span>
                ...
                <span className={clsx(
                  ...
                  isSelected && !isToday  ? 'text-brand-700 dark:text-brand-200' : '',
                  !isSelected && isToday  ? 'text-brand-600' : '',
                ...
                {hasTasks && (
                  <div className={clsx(
                    ...
                    !allDone && isSelected && isToday  ? 'bg-white/70' : '',
                    !allDone && isSelected && !isToday ? 'bg-brand-400' : '',
                    !allDone && !isSelected ? 'bg-brand-300 dark:bg-brand-600' : '',
                  )} />
                )}
              </button>
            )
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-sm font-black text-gray-700 dark:text-gray-300">
            {diaSelecionado === hojeKey
              ? 'Hoje'
              : diaSelecionado === toKey(new Date(hoje.getTime() - 86400000))
              ? 'Ontem'
              : diaSelecionado === toKey(new Date(hoje.getTime() + 86400000))
              ? 'Amanhã'
              : fromKey(diaSelecionado).toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })
            }
          </p>
          <p className="text-xs text-gray-400 font-semibold">
            {fromKey(diaSelecionado).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' })}
          </p>
        </div>
        {tarefasDia.length > 0 && (
          <span className="text-xs font-bold text-brand-600 bg-brand-50 dark:bg-brand-950 px-3 py-1.5 rounded-xl">
            {concluidas}/{tarefasDia.length} · {pctDia}%
          </span>
        )}
      </div>
        ...
        <div className="space-y-2 mb-4">
          {tarefasDia.map(t => (
            <div
              key={t.id}
              className={clsx(
                'card flex items-center gap-3 py-3 transition-all',
            ...
            >
              <button onClick={() => toggle(t.id)} className="flex-shrink-0">
                {t.status === 'done'
                  ...
                  : t.status === 'next'
                  ? <Clock size={22} className="text-brand-500 animate-pulse" />
              ...
              <button onClick={() => abrirEditar(t)} className="p-1.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 flex-shrink-0">
                <Edit2 size={15} className="text-gray-400" />
              </button>
              <button onClick={() => excluir(t.id)} className="p-1.5 rounded-xl hover:bg-red-50 dark:hover:bg-red-950 flex-shrink-0">
      ...
      <button onClick={abrirNova} className="w-full btn-primary flex items-center justify-center gap-2">
        <Plus size={18} />
        Nova tarefa {diaSelecionado !== hojeKey && `— ${fromKey(diaSelecionado).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}`}
      </button>

      {/* ── Modal adicionar/editar ── */}
      {modal && (
        <Modal
          titulo={modal === 'nova' ? 'Nova tarefa' : 'Editar tarefa'}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-brand-50 dark:bg-brand-950 rounded-2xl px-4 py-2.5">
              <p className="text-xs font-bold text-brand-600 dark:text-brand-400">
                📅 {diaSelecionado === hojeKey ? 'Hoje' : fromKey(diaSelecionado).toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })}
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {CATEGORIAS.map(c => (
                  <button key={c} onClick={() => setForm(p => ({ ...p, cat: c }))} className={chip(form.cat === c)}>
                    {c}
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={() => setModal(null)} className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 text-sm font-bold text-gray-500">
                Cancelar
              </button>
              <button onClick={salvar} className="flex-1 btn-primary py-3 flex items-center justify-center gap-2">
