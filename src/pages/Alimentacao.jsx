...
import { Plus, Droplets, ChevronDown, ChevronRight, Trash2, Edit2 } from 'lucide-react'
...
const refeicoesDefault = [
  ...
  {id:3,nome:'Lanche',emoji:'🍎',hora:'16:00',alimentos:[{id:31,nome:'Whey protein',cals:130,prot:25,carb:5,gord:2}]},
...
export default function Alimentacao() {
  ...
  const abrirAl = (refId,al=null) => {
    setFAl(al?{nome:al.nome,cals:al.cals,prot:al.prot,carb:al.carb,gord:al.gord}:{nome:'',cals:'',prot:'',carb:'',gord:''})
    setModalAl({refId,alId:al?al.id:'novo'})
  }
  const salvarAl = () => {
    if (!fAl.nome) return
  ...
  const abrirRef = (r=null) => {
    setFRef(r?{nome:r.nome,emoji:r.emoji,hora:r.hora}:{nome:'',emoji:'🍽️',hora:'12:00'})
    setModalRef(r?r.id:'novo')
  }
  const salvarRef = () => {
    if (!fRef.nome) return
    ...
    <div className="page-container animate-fade-in-up">
      ...
      <div className="card mb-5">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2"><Droplets size={20} className="text-blue-500"/><span className="font-bold text-gray-700 dark:text-gray-300">Hidratação</span></div>
        ...
        {refeicoes.map(r=>(
          <div key={r.id} className="card">
            <div className="flex items-center gap-3">
              <button onClick={()=>setAbertas(p=>({...p,[r.id]:!p[r.id]}))} className="flex items-center gap-3 flex-1 text-left">
              ...
              <div className="mt-3 pt-3 border-t border-gray-50 dark:border-gray-800 space-y-2">
                ...
                {r.alimentos.map(a=>(
                  <div key={a.id} className="flex items-center gap-2 py-1">
                    ...
                    <button onClick={()=>setRefeicoes(p=>p.map(rx=>rx.id!==r.id?rx:{...rx,alimentos:rx.alimentos.filter(x=>x.id!==a.id)}))} className="p-1 rounded-lg hover:bg-red-50 dark:hover:bg-red-950"><Trash2 size={13} className="text-red-400"/></button>
                  </div>
                ))}
                <button onClick={()=>abrirAl(r.id)} className="w-full flex items-center gap-2 py-2 text-brand-500 font-bold text-xs hover:text-brand-600"><Plus size={14}/> Adicionar alimento</button>
              </div>
            )}
          </div>
        ))}
      ...
      {modalAl && (
        <Modal titulo={modalAl.alId==='novo'?'Adicionar alimento':'Editar alimento'} onClose={()=>setModalAl(null)} onConfirm={salvarAl} confirmLabel="Salvar alimento">
          <div className="space-y-3">
            <div><label className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1 block">Nome do alimento</label><input type="text" value={fAl.nome} onChange={e=>setFAl(p=>({...p,nome:e.target.value}))} placeholder="Ex: Frango grelhado 150g" className="input" autoFocus/></div>
      ...
      {modalRef && (
        <Modal titulo={modalRef==='novo'?'Nova refeição':'Editar refeição'} onClose={()=>setModalRef(null)} onConfirm={salvarRef} confirmLabel="Salvar refeição">
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-3 flex-wrap">{EMOJIS_R.map(em=><button key={em} onClick={()=>setFRef(p=>({...p,emoji:em}))} className={clsx('text-2xl w-10 h-10 rounded-xl flex items-center justify-center',fRef.emoji===em?'bg-brand-100 dark:bg-brand-900 ring-2 ring-brand-500':'hover:bg-gray-100 dark:hover:bg-gray-800')}>{em}</button>)}</div>
            </div>
          </div>
        </Modal>
      )}
      ...
      {modalMetas && (
        <Modal titulo="Metas nutricionais" onClose={()=>setModalMetas(false)} onConfirm={()=>{setMetas(fMetas);setModalMetas(false)}} confirmLabel="Salvar metas">
          <div className="space-y-3">
