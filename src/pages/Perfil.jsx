...
import {
  ...
  CheckCircle2, Bell, Shield, Download, LogOut, Camera,
  X, Check, Edit2, Mail, Phone, MapPin, Calendar, ArrowLeft,
  Lock, Eye, EyeOff, FileText, AlertTriangle, RefreshCw, Sun, Moon, Monitor
} from 'lucide-react'
import clsx from 'clsx'
import { clearAllStorage } from '../hooks/useLocalStorage'
  ...
  return (
    <SubPage title="Editar Perfil" onBack={onBack}>
      ...
      <div className="card mb-4 flex flex-col items-center py-6">
        ...
        <button onClick={() => setAvatarOpen(true)} className="flex items-center gap-1.5 text-brand-600 font-bold text-sm">
            ...
            <div className="grid grid-cols-8 gap-3 pb-2">
              {AVATARES.map(av => (
                <button key={av} onClick={() => { set('avatar', av); setAvatarOpen(false) }}
                  className={clsx('text-2xl w-10 h-10 rounded-2xl flex items-center justify-center transition-all',
                    form.avatar === av ? 'bg-brand-100 dark:bg-brand-900 ring-2 ring-brand-500' : 'hover:bg-gray-100 dark:hover:bg-gray-800')}>
                  {av}
                </button>
              ))}
            </div>
          ...
          return (
            <div key={f.key}>
              ...
              <div className="relative">
                <Icon size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300 dark:text-gray-600" />
                <input type={f.type} value={form[f.key] || ''} onChange={e => set(f.key, e.target.value)} placeholder={f.placeholder} className="input pl-9" />
              </div>
            </div>
          )
        })}
      ...
      <div className="card mb-5">
        ...
        <textarea value={form.bio || ''} onChange={e => set('bio', e.target.value)}
          placeholder="Escreva algo sobre você ou seu objetivo..." rows={3} className="input resize-none" />
        <p className="text-xs text-gray-300 dark:text-gray-600 font-semibold mt-1 text-right">{(form.bio || '').length}/150</p>
      </div>
...
function Notificacoes({ onBack }) {
  const [cfg, setCfg] = useState(() => {
    try { return JSON.parse(localStorage.getItem('evoluaja_notif') || 'null') || { habitos: true, treinos: true, financas: false, agua: true, estudo: false, sono: true, resumoDiario: true, horarioResumo: '20:00' } }
    catch { return { habitos: true, treinos: true, financas: false, agua: true, estudo: false, sono: true, resumoDiario: true, horarioResumo: '20:00' } }
  })
  const [salvo, setSalvo] = useState(false)
  const toggle = k => setCfg(p => ({ ...p, [k]: !p[k] }))
  const salvar = () => {
    localStorage.setItem('evoluaja_notif', JSON.stringify(cfg))
...
function Privacidade({ onBack }) {
  const [cfg, setCfg] = useState({ perfilPublico: false, mostrarStreak: true, mostrarStats: true, compartilharProgresso: false })
  ...
  const InputSenha = ({ campo, label }) => (
    <div>
      ...
      <div className="relative">
        ...
        <input type={verSenha[campo] ? 'text' : 'password'} value={senhaForm[campo]}
...
function ExportarDados({ onBack }) {
  ...
  const exportar = (id, key) => {
    ...
    setTimeout(() => {
      ...
      a.href = url; a.download = `evoluaja-${id}-${new Date().toLocaleDateString('pt-BR').replace(/\//g,'-')}.json`; a.click()
  ...
  return (
    <SubPage title="Exportar Dados" onBack={onBack}>
      <div className="card mb-4 bg-blue-50 dark:bg-blue-950 border-blue-100 dark:border-blue-900">
        ...
        <p className="text-xs text-blue-600 dark:text-blue-400 font-semibold">Exporte a qualquer momento em JSON — compatível com Excel e Google Sheets.</p>
      </div>
        ...
        {modulos.map(m => (
          <div key={m.id} className="card flex items-center gap-4">
            ...
            <button onClick={() => exportar(m.id, m.key)} disabled={exportando === m.id}
              className={clsx('flex-shrink-0 w-10 h-10 rounded-2xl flex items-center justify-center transition-all',
                exportados.includes(m.id) ? 'bg-emerald-100 dark:bg-emerald-900' : 'bg-brand-100 dark:bg-brand-900 hover:bg-brand-200')}>
              {exportando === m.id
...
function ResetGeral({ onBack }) {
  ...
  const resetar = () => {
    setResetando(true)
    setTimeout(() => {
      // Limpa TUDO incluindo streak e last_active
      clearAllStorage()
      localStorage.removeItem('evoluaja_streak')
      localStorage.removeItem('evoluaja_last_active')
  ...
  return (
    <SubPage title="Reiniciar App" onBack={onBack}>
      <div className="card mb-5 flex flex-col items-center py-10 text-center">
        ...
        <h2 className="text-lg font-black text-gray-800 dark:text-gray-100 mb-3">Reiniciar do zero</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 font-semibold max-w-xs leading-relaxed">
          Todos os dados serão apagados permanentemente:
        </p>
        <div className="mt-3 space-y-1 text-xs text-gray-400 font-semibold text-left">
          {['🔥 Streak e contagem de dias ativos','✅ Hábitos e rotinas','💰 Finanças e transações','💪 Fichas de treino','🥗 Refeições e macros','📚 Matérias e estudos','🏠 Tarefas de casa','❤️ Consultas e medicamentos','⚙️ Configurações e perfil'].map(item => (
            <p key={item}>{item}</p>
          ))}
        </div>
        <p className="mt-4 font-black text-red-500 text-sm">Esta ação não pode ser desfeita.</p>
      </div>
      ...
      {feito ? (
        ...
        <div className="card border-red-100 dark:border-red-900 bg-red-50 dark:bg-red-950">
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-sm font-bold text-red-700 dark:text-red-300">Tem certeza? Todos os dados serão perdidos!</p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => setConfirmando(false)}
              className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm font-bold text-gray-600 dark:text-gray-400">
              Cancelar
            </button>
            <button onClick={resetar} disabled={resetando}
              className="flex-1 py-3 rounded-2xl bg-red-500 text-white text-sm font-bold flex items-center justify-center gap-2 hover:bg-red-600">
              {resetando
...
function SairConta({ onBack }) {
  ...
  const sair = () => {
    setSaindo(true)
    setTimeout(() => { setSaindo(false); setConfirmando(false); onBack() }, 1500)
  }
    ...
    <SubPage title="Sair da Conta" onBack={onBack}>
      <div className="card mb-5 flex flex-col items-center py-10 text-center">
        ...
        <p className="text-sm text-gray-400 font-semibold max-w-xs leading-relaxed">
          Você será desconectado. Seus dados ficam salvos e você pode entrar novamente quando quiser.
        </p>
      </div>
      {!confirmando ? (
        <button onClick={() => setConfirmando(true)}
          className="w-full py-4 rounded-2xl bg-red-500 text-white font-bold flex items-center justify-center gap-2 hover:bg-red-600 shadow-md shadow-red-100">
        ...
        <div className="card border-red-100 dark:border-red-900 bg-red-50 dark:bg-red-950">
          <div className="flex items-center gap-3 mb-4">
            <AlertTriangle size={20} className="text-red-500 flex-shrink-0" />
            <p className="text-sm font-bold text-red-700 dark:text-red-300">Tem certeza que deseja sair?</p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => setConfirmando(false)}
              className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm font-bold text-gray-600 dark:text-gray-400">
...
export default function Perfil() {
  ...
  // Usa AppContext — perfil compartilhado com Dashboard
  const { perfil, setPerfil, streak, frase, animando, registrarUso } = useApp()
  ...
  const configItens = [
    ...
    { key: 'reset',        label: 'Reiniciar do zero', icon: RefreshCw, desc: 'Apagar todos os dados e streak', danger: true },
  ...
  return (
    <div className="page-container animate-fade-in-up">
      ...
      {/* Avatar e nome — atualiza em tempo real via AppContext */}
      <div className="flex flex-col items-center mb-6 pt-2">
        <div className="relative mb-3">
          ...
          <button onClick={() => setPagina('editar')}
            className="absolute bottom-0 right-0 w-8 h-8 bg-white dark:bg-gray-800 rounded-full border-2 border-brand-200 flex items-center justify-center shadow-sm hover:bg-brand-50">
      ...
      <div className="card mb-5">
        ...
        <div className="flex gap-2 bg-gray-100 dark:bg-gray-800 rounded-2xl p-1">
          {[
            ...
            <button key={id}
              onClick={() => id === 'system' ? setSystem() : theme !== id ? toggle() : null}
      ...
      <div className="card mb-5">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-10 h-10 bg-brand-100 dark:bg-brand-900 rounded-2xl flex items-center justify-center">
            <Sparkles size={20} className="text-brand-600" />
          </div>
          <div>
            <h2 className="font-black text-gray-800 dark:text-gray-100">IA de Rotina Automática</h2>
            ...
            return (
              <div key={q.key}>
                <div className="flex items-center gap-2 mb-2.5">
                  ...
                  <p className="text-sm font-bold text-gray-700 dark:text-gray-300">{q.label}</p>
                </div>
        ...
        <button onClick={gerarRotina} disabled={!todosRespondidos || gerando}
          className={clsx('w-full mt-5 btn-primary flex items-center justify-center gap-2', !todosRespondidos && 'opacity-40 cursor-not-allowed')}>
          {gerando
          ...
          {configItens.map(item => {
            ...
            return (
              <button key={item.key} onClick={() => setPagina(item.key)}
                className={clsx('w-full flex items-center gap-3 py-3 px-2 rounded-2xl transition-all text-left',
                  item.danger ? 'hover:bg-red-50 dark:hover:bg-red-950' : 'hover:bg-gray-50 dark:hover:bg-gray-800')}>
                <div className={clsx('w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0',
                  item.danger ? 'bg-red-100 dark:bg-red-950' : 'bg-gray-100 dark:bg-gray-800')}>
