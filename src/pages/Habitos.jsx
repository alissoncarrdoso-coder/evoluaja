...
export default function Habitos() {
  const [habitos, setHabitos] = useLocalStorage('evoluaja_habitos', habitosDefault)
  ...
  const salvar = () => {
    ...
    if (modal === 'novo') setHabitos(prev => [...prev, { ...form, id: Date.now(), done: false }])
  ...
  return (
    <div className="page-container animate-fade-in-up">
      ...
      <div className="space-y-2 mb-5">
        {habitos.map(h => (
          <div key={h.id} className={clsx('card flex items-center gap-3 py-3 transition-all', h.done ? 'bg-emerald-50 dark:bg-emerald-950 border-emerald-100 dark:border-emerald-900' : '')}>
            <button onClick={() => toggle(h.id)} className="flex-shrink-0">
              {h.done ? <CheckCircle2 size={22} className="text-emerald-500" /> : <Circle size={22} className="text-gray-300 dark:text-gray-600" />}
            </button>
            <span className="text-xl flex-shrink-0">{h.emoji}</span>
            <span className={clsx('flex-1 text-sm font-bold', h.done ? 'line-through text-gray-400 dark:text-gray-600' : 'text-gray-800 dark:text-gray-100')}>{h.nome}</span>
              ...
              <div className="grid grid-cols-10 gap-2">
                {EMOJIS.map(em => (
                  <button key={em} onClick={() => setForm(p => ({ ...p, emoji: em }))}
                    className={clsx('text-xl w-9 h-9 rounded-xl flex items-center justify-center transition-all', form.emoji === em ? 'bg-brand-100 dark:bg-brand-900 ring-2 ring-brand-500' : 'hover:bg-gray-100 dark:hover:bg-gray-800')}>
                    {em}
                  </button>
                ))}
              </div>
